...
export class GitService {
  ...
  getFileTree( projectName: string, folder?: string ): Observable<any> {
    if ( folder === undefined || folder === null ) {
      return this.http.get<any>(environment.api_url + '/' + this.apiUrlPath + '/' + projectName + '/get_tree/');
    } else {
      folder = folder.slice(0, folder.lastIndexOf('/'));
      return this.http.get<any>(environment.api_url + '/' + this.apiUrlPath + '/' + projectName + '/get_tree/' + folder );
    }
  }
}
