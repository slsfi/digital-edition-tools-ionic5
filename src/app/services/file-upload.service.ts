...
/**
 * https://stackblitz.com/edit/angular-7-file-upload-queue?file=app%2Ffile-uploader.service.ts
 */
...
export class FileUploadService {
  ...
  private facsimileCollectionId: string;
  private projectName: string;
  private imageStartNumber: number;
  ...
  public setImageStartNumber ( startNumber: number ) {
    this.imageStartNumber = startNumber;
  }
  ...
  public addToQueue(data: any) {
    ...
    _.each(data,
      (file: any) => {
        this._addToQueue(file, index);
        index++;
      }
    );
  }

  public setFacsimileCollectionId( id: string ) {
    this.facsimileCollectionId = id;
  }
  ...
  // private functions
  private _addToQueue(file: any, index: number) {
    const queueObj = new FileQueueObject(file);

    // set the individual object events
    queueObj.upload = () => this._upload(queueObj, index);
  ...
  private _upload(queueObj: FileQueueObject, fileNumber:number) {
    // create form data for file
    const form = new FormData();
    form.append('facsimile', queueObj.file, queueObj.file.name);
